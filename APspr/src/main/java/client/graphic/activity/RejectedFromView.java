...
import client.controller.RejectedFromController;
...
public class RejectedFromView implements MenuButton, CloseButton, BackToActivityButton, ScrollPaneFromBottom {

    public void rejectedFrom(LinkedList<String> rejectedFrom){
        ...
        for (String idiot : rejectedFrom){
            JLabel nameLabel = new JLabel();
            try {
                FileReader reader = new FileReader("resources/configuration/activityButtons.properties");
                Properties properties = new Properties();
                properties.load(reader);
                nameLabel.setFont(new Font(properties.getProperty("fontName"),Font.PLAIN,
                        Integer.parseInt(properties.getProperty("fontSize"))));
                nameLabel.setText(idiot + " " + properties.getProperty("nameLabelText"));
            }catch (Exception e){
                e.printStackTrace();
            }
            panel.add(nameLabel);
